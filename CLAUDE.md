...
### 終わった枠を表示しない
- 今日の一覧では、開始時刻を過ぎた枠を表示しない（表示する時点の時刻で判定する）
- 判定は取得時刻ではなく閲覧時刻で行う。データは数十分前のスナップショットで、開きっぱなしでも古くならないようにするため
- 昨日・明日以降を選んでいるときは対象外（そのまま全部出す）
- 実装: `public/index.html` の `getSlotDeadline()` / `isSlotUpcoming()`。1分ごとに枠数を見て、減っていたら描き直す
...
### 枠の並び順と表記（全施設共通）
- **並び順は実際の時刻順**にする。予約サイトが返す順をそのまま使わない（GIRAFFEは朝の枠が最後に、KUDOCHIは早朝の枠が最後に来ていた）
- **終了が翌日にまわる枠は「翌0:10」と表記**する。予約サイトによって `22:10〜24:10` と `22:00〜0:00` の両方の書き方で来るため、表示側でそろえる
- **時刻はゼロ埋めしない**。`9/7 0:40〜2:40` と書き、`9/7 00:40〜02:40` としない（通常の枠が `8:20〜9:50` 表記のため）
- **時間帯の絞り込み（朝/昼/夜/深夜）は全ての表記に効かせる**。`9/7 1:00〜8:00`（脈のナイトパック）は翌日の枠として24時以降扱い、`13時代全て`（BASE）は13時として扱う
- 実装: `public/index.html` の `getSlotStartTime()` / `compareSlots()` / `getSlotStartHour()`
...
- Cloudflare保護なし
- カレンダーのDOM構造がシンプル
- 空き判定: `.d_lesson:not(.disabled)` かつ `full` クラスなし
- **終わった枠がしばらく空きのまま残る**: 終了から1時間半以上たっても `disabled` が付かない枠があった（2026-09-06に実測: 8:20〜9:50 の枠が 11:17 時点でも空き扱い）。スクレイパーはそのまま取り込むので、過ぎた枠は表示側で落とす（「終わった枠を表示しない」参照）
- **ナイトパック（5時間パック）は翌日の列に並ぶが、実際は前の夜に入る枠**: 例えば 9/7 の列の `0:00〜5:00` は 9/6 の夜に入る。GIRAFFE・脈と同じく「入る夜」の日付にまとめ、時刻は `24:00〜29:00` と24時以降の表記に変換して取り込む（`toLateNightNotation`）
- **通常プランの早朝枠（5:40〜7:40 等）は当日のまま**にする。朝サウナとして当日の朝に入る枠なので、ナイトパックと混同しない
