...
- **終わった枠がしばらく空きのまま残る**: 終了時刻の1時間後くらいまで `disabled` が付かない（2026-09-06に実測）。スクレイパーはそのまま取り込むので、過ぎた枠は表示側で落とす（「終わった枠を表示しない」参照）
- **0〜8時台の枠はその日の早朝**（翌日ではない）: カレンダーの各日付の列に 0:00〜5:00 のナイトパックや 5:40〜7:40 の早朝枠が並ぶ。GIRAFFEの `24:40〜26:40`（選択日の夜→翌未明）とは意味が違うので、日付を頭に付けない
